...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        echo "Erro ao deletar contato: " . $stmt->error;
    }
    $stmt->close();
    $conn->close();
} else {
    // Confirmar exclusão
    echo "<p>Tem certeza que deseja excluir este contato?</p>";
    echo "<form method='POST' action='deleta_contato.php?id=$id'>";
    echo "<button type='submit' class='btn btn-danger'>Deletar</button>";
    echo "<a href='lista_contatos.php' class='btn btn-secondary'>Cancelar</a>";
    echo "</form>";
}
?>
